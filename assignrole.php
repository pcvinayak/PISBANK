...
<body>
    ...
    <div>
            <?php 
            $query="SELECT role_Id, role from d_roles where role_Id <> 1";
            $result=mysqli_query($conn,$query);
            $dropdownroles="<select name='dropdownlistroles'>";
            while($row = mysqli_fetch_array($result)) 
            {
                $dropdownroles=$dropdownroles."<option value='".$row[0]."'>".$row[1]."</option>";
            }
            $dropdownroles=$dropdownroles."</select>";
